...
if($_POST[tambah]){
if(!empty($fname) and  !empty($lname) and  !empty($province) and !empty($city) and !empty($address) and  !empty($phone) and  !empty($gelar) and !empty($description)  and !empty($username)){
...
$add=mysql_query("UPDATE `author` set `fname`='$fname', `lname`='$lname', `province`='$province', `city`='$city', `address`='$address', `phone`='$phone', `gelar`='$gelar', `description`='$description', `photo`='$filefoto', `username`='$username', `password`='$password' where id='$id'");
}else{
echo "max upload 1mb";
}
}else{
$add=mysql_query("UPDATE `author` set `fname`='$fname', `lname`='$lname', `province`='$province', `city`='$city', `address`='$address', `phone`='$phone', `gelar`='$gelar', `description`='$description', `username`='$username' where id='$id'");
}
if($add){
if($_SESSION['loginas']==9){
echo "<center><font color=\"green\">*) Berhasil Update Data Author </center></font>";
...
echo "<meta http-equiv='refresh' content='2; url=index.php?p=detailauthor'; />";
}
}else{
echo mysql_error();
}
