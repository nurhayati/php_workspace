...
if($_POST[tambah]){
if(!empty($fname) and  !empty($lname) and  !empty($province) and !empty($city) and !empty($address) and  !empty($phone) and !empty($description) and !empty($filefoto) and !empty($username) and !empty($password)){
if(is_numeric($phone)){
...
$add=mysql_query("INSERT INTO `member` (`id`, `fname`, `lname`, `province`, `city`, `address`, `phone`, `description`, `photo`, `username`, `password`) VALUES ('$random', '$fname', '$lname', '$province', '$city', '$address', '$phone', '$description', '$filefoto', '$username', '$password')");
}else{
echo "max upload 1mb";
}
if($add){
if($_SESSION['loginas']==9){
echo "<center><font color=\"green\">*) Berhasil Tambahkan member Baru</center></font>";
echo "<meta http-equiv='refresh' content='2; url=index.php?p=adminmember'; />";
}else{
echo "<center><font color=\"green\">*) Anda Berhasil Bergabung Menjadi Member</center></font>";
